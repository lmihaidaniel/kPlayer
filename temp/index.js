...
var tinycolor$1 = interopDefault(tinycolor);
...
var Events = interopDefault(index$1);
...
class Player extends Video {
	constructor(settings) {
		...
		this.externalControls.enabled(settings['externalControls']);

		this.settings(deepmerge(defaults$8, settings));
...
class videoPopup extends Popup {
	constructor(el, opts, parent, parentPlayer) {
		...
		this.player.cancelFullWindow = () => {
			...
			this.header.backgroundColor(this._settings.headerBg, this._settings.headerBgAlpha);
...
function Timeline$1 (parentPlayer, options) {
	return function () {
		let Timeline = function () {
			let settings = deepmerge(defaults$11, options);
			let fragment = document.createDocumentFragment();
			let svg = new svgSprite();
			let playBtn = createElement('button');
			let playBtnSVG = svg.create('play', playBtn);
			...
			this.setChapters = (chapters, equalChapters, duration) => {
				this.chapters = new Chapters(parentPlayer, chapters, equalChapters, duration);
			};
		};
		return new Timeline();
	}();
...
class controlsWidget {
	constructor(parentPlayer, options) {
		let temp = deepmerge(defaults$12, {
			x: parentPlayer.videoWidth() - defaults$12.width / 2,
			y: (parentPlayer.videoHeight() - defaults$12.height) / 2
		});
