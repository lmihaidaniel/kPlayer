import style from './container.sss';
import {createElement, hasClass, addClass, removeClass, toggleClass, isEl} from '../../helpers/dom';
...
export default class Containers {
	constructor(parentPlayer) {
		...
		this.addCustom = function(Construct, settings, el = {}){
			let cls = settings.className || '';
			let containerBody = createElement('div');
			let container = null;
			addClass(containerBody, 'kmlPopup isVideo hidden ' + cls);
			container = new Construct(containerBody, settings, this, parentPlayer, el);
		...
		this.addCustomSafe = function(container) {
			popups.appendChild(container.wrapper);
			this._els.push(container);
			return container;
		}
	...
	up(a){
		let childs = this.wrapper.children;
		if(childs[a]){
			this.wrapper.insertBefore(childs[a],childs[a].previousSibling || null);
		}
	}
	...
	down(a){
		let childs = this.wrapper.children;
		if(childs[a]){
			this.wrapper.insertBefore(childs[a],childs[a].nextSibling || null);
		}
	}
