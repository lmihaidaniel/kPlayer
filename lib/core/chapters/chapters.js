...
export default class Chapters {
	...
	set(settings, equalVisuals = true, _d) {
		this.destroy();
		let duration = _d || this.player.duration();
		settings = settings || [{
			start: 0
		}];
		let progressDivs = [];
		...
		for (var k in settings) {
			...
			if (visual.icons != null){
				for(let v in visual.icons){
					let vd = visual.icons[v];
					if(vd.timestamp >= meta.start && vd.timestamp <= meta.end){
						var icon = document.createElement('div');
						icon.className = "kmlCuepoint-marker-point";
						icon.style.left = (vd.timestamp - meta.start)/(meta.end-meta.start)*100+"%";
						if(vd.icon){
							icon.style.backgroundImage = vd.icon;
						}
						visual.content.appendChild(icon);
					}
				}
				visual.icons = null;
			}
