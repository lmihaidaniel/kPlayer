...
export default class Chapters {
	...
	set(_chapters, equalVisuals = true, _d) {
		...
		function create_items_menu(items, icon){
			let n = items.length;
			let menu_wrapper = createElement('div', {class:"menu"});
			for (var k in items) {
				let item = items[k];
				let _item = createElement('div', {class:"item"});
				if (item.icon) {
					_item.style.backgroundImage = 'url(' + item.icon + ')';
				}
				if (item.label) {
					let _label = createElement('span', {class:"label"}, item.label);
					_item.appendChild(_label);
				}
				for(var j in item.on){
					_item.addEventListener(j, item.on[j]);
				}
				menu_wrapper.appendChild(_item);
			}
			icon.appendChild(menu_wrapper);
		}
		...
		for (var k in _chapters) {
			...
			k = parseInt(k);
			let offsetStart = _chapters[k].start;
			...
			if (visual.items != null) {
				...
				for (let v in visual.items) {
					...
					if (vd.start >= meta.start && vd.start <= meta.end) {
						...
						if (vd.menu) {
							create_items_menu(vd.menu, icon);
						}
