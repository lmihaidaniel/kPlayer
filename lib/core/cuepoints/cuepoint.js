import style from './cuepoints.sss';
...
export default class Cuepoint extends Events {
    constructor(parentPlayer, options, parentWrapper) {
        ...
        for (var k in this.__settings['on']) {
            if (isFunction(this.__settings['on'][k])) {
                this.on(k, this.__settings['on'][k], this);
            }
        }
    ...
    processHandler(td) {
        if (td) {
            ...
            this.__fired = true;
            if (!this.__onceStart) {
                this.emit('start',this.__settings); //Call start function
                if (this.el) {
    ...
    process(tc) {
        if (tc >= this.__settings['start'] && (this.__settings['end'] < 0 || tc < this.__settings['end'])) {
            this.__settings['on']['process'](tc);
    ...
    activate() {
        this.__tId = true;
        if (this.__settings['raf']) {
            if (!this.__process) {
                this.__process = () => {
                    if (this.__tId) {
                        this.process(this.parentPlayer.media.currentTime);
            ...
            this.parentPlayer.on('timeupdate', this.process, this);
        }
        this.emit('activate', this.__settings);
    }
    suspend() {
        this.__tId = false;
        if (this.__settings['raf']) {
            ...
            this.parentPlayer.off('timeupdate', this.process, this);
        }
        this.emit('suspend', this.__settings);
    }
    destroy() {
        this.suspend();
        if (this.el) {
        ...
        this.emit('destroy', this.__settings);
        this.__fired = false;
    }
    addVisual(visual) {
        if (!this.el) {
