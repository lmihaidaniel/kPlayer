...
export default function(parentPlayer, options) {
	return (function() {
		let Timeline = function() {
			...
			parentPlayer.cuepoints.changeWrapper(pc);
			let wrapper = null;
			...
			this.resize = () => {
				...
				if (cache_sc != sc && this.wrapper.visible()) {
					...
					if (sc < settings.minHeight) {
						...
						parentPlayer.emit('timelineHeight', settings.minHeight + "px");
						...
						parentPlayer.emit('timelineHeight', settings.height);
			...
			this.closed = (v) => {
				...
				parentPlayer.emit('timelineClosed', this, _closed);
				return _closed;
			}
			...
			parentPlayer.on('timelineCheckSize', ()=>{
				this.resize();
				parentPlayer.emit('timelineHeight', timelineHeight);
				this.closed();
			});
			...
			this.show = ()=>{
				parentPlayer.emit('timelineShow');
				this.closed(false);
				this.wrapper.show();
			}
			this.hide = ()=>{
				parentPlayer.emit('timelineHide');
				this.wrapper.hide();
			}

			let chapters = new Chapters(parentPlayer, null, true, null, this);
			this.chapters = (settings, equalChapters, duration)=>{
				chapters.set(settings, equalChapters, duration);
			}
		}
		return new Timeline();
	})();
